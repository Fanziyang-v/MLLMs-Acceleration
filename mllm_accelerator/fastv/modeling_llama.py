    ...
    **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
    ...
    for layer_idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
        ...
        if is_prefill: # FastV applies visual token pruning in the prefilling stage.
            ...
            # Obtain FastV parameters
            fastv_k = self.fastv_info["fastv_k"]
            fastv_r = self.fastv_info["fastv_r"]
            visual_token_start_index = self.fastv_info["visual_token_start_index"]
            visual_token_length = self.fastv_info["visual_token_length"]
            visual_token_end_index = visual_token_start_index + visual_token_length
            num_retained_tokens = int(fastv_r * visual_token_length)
            if layer_idx == fastv_k - 1:
                ...
                # Get `keep_indices`
                device, dtype = topk_indices.device, topk_indices.dtype
                pre_visual_indices = torch.arange(visual_token_start_index, device=device, dtype=dtype)
